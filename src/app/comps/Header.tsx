  ...
  return (
    <div className="flex items-center justify-between px-[5%] py-4 relative">
      ...
      <div className="hidden lg:flex items-center gap-5">
        <div onClick={() => handleNavigation('/', 'Home')} className={getTabClass('Home')}>Home</div>
        ...
        <div onClick={() => handleNavigation('/faqs', 'FAQs')} className={getTabClass('FAQs')}>FAQs</div>
      </div>
          ...
          user ? (
            ...
            (
              <>
                ...
                <div
                  onClick={() => handleNavigation('/sign-in', 'Order')}
                  className="bg-yellow-300 px-5 py-1 rounded-[30px] font-bold cursor-pointer"
                >
                  Sign in
                </div>
                <div
                  onClick={() => handleNavigation('/sign-up', 'Order')}
                  className="bg-yellow-500 px-5 py-1 rounded-[30px] font-bold cursor-pointer"
                >
                  Sign up
                </div>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="absolute top-[70px] left-0 w-full bg-white px-[5%] py-4 flex flex-col gap-4 shadow-md z-1000 lg:hidden">
          <div onClick={() => handleNavigation('/user/home', 'Home')} className={getTabClass('Home')}>Home</div>
          <div onClick={() => handleNavigation('/user/menu', 'Menu')} className={getTabClass('Menu')}>Menu</div>
          <div onClick={() => handleNavigation('/user/reservation', 'Reservation')} className={getTabClass('Reservation')}>Reservation</div>
          <div onClick={() => handleNavigation('/user/contact', 'Contact')} className={getTabClass('Contact')}>Contact</div>
          <div onClick={() => handleNavigation('/user/order', 'Order')} className="bg-yellow-500 px-5 py-1 rounded-[30px] font-bold cursor-pointer w-fit">Order</div>
          {
            user ? (
              <>
                <button
              ...
              <div
                onClick={() => handleNavigation('/sign-in', 'Order')}
                className="bg-yellow-300 px-5 w-full text-center py-1 rounded-[30px] font-bold cursor-pointer"
              >
                Sign in
              </div>
              <div
                onClick={() => handleNavigation('/sign-up', 'Order')}
                className="bg-yellow-500 px-5 py-1 w-full text-center  rounded-[30px] font-bold cursor-pointer"
              >
                Sign up
              </div>
            </div>
